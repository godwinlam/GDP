...
interface FiveHundredRewardScreenProps {
  user: User;
}

// Update component definition to use props
export default function FiveHundredRewardScreen({
  user,
}: FiveHundredRewardScreenProps) {
  ...
  const checkEligibility = async () => {
    try {
      ...
      setRewardAmount(parentGDPPrice * 4.0);
      ...
      const condition2 = matchingChildren.length >= 12;
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        transaction.update(gdpPurchaseRef, {
          rewardClaimed500: true,
        });

        // Create new GDP purchase for OTC
        ...
        await addDoc(transactionRef, {
          ...
          description:
            "500% GDP Reward for having 9+ children, 4+ grandchildren, 8+ great-grandchildren, 16+ great-great-grandchildren, and 32+ great-great-great-grandchildren with matching GDP",
        });
      });
  ...
  return (
    ...
    >
      <Text style={styles.title}>500% GDP {t.reward}</Text>

      {rewardClaimed ? (
        <View>
          <Text style={styles.claimedText}>
            {t.You_have_already_claimed_your} 500% GDP {t.reward}.
          </Text>
        </View>
      ) : eligible ? (
        <View>
          <Text style={styles.eligibleText}>
            {t.Congratulations}! {t.You_are_eligible_for_the} 500% GDP {t.reward}!
          </Text>
        ...
        <View>
          <Text style={styles.ineligibleText}>
            {t.You_are_not_eligible_for_the} 500% GDP {t.reward}
          </Text>
          <Text style={styles.requirementText}>{t.requirements} ({t.either}1 || 2) :</Text>
          <Text style={styles.bulletPoint}>
            • {t.condition} 1: {t.at_least} 8 {t.friend} {t.and} 4 + 8 + 16 + 32 {t.friend} {t.ecological_relations} {t.with} {t.equal} GDP
          </Text>
          <Text style={styles.bulletPoint}>
            • {t.condition} 2: {t.at_least} 12 {t.friend} {t.with} {t.equal} GDP
          </Text>
          <Text style={styles.bulletPoint}>
