...
interface ThreeHundredRewardScreenProps {
  user: User;
}

// Update component definition to use props
export default function ThreeHundredRewardScreen({
  user,
}: ThreeHundredRewardScreenProps) {
  ...
  const checkEligibility = async () => {
    try {
      ...
      if (parentGDPPurchase.rewardClaimed300) {
      ...
      setRewardAmount(parentGDPPrice * 2.0);
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        await addDoc(transactionRef, {
          ...
          description:
            "300% GDP Reward for having 6+ children, 4+ grandchildren, 8+ great-grandchildren, and 16+ great-great-grandchildren with matching GDP",
        });
      });
  ...
  return (
    ...
    >
      <Text style={styles.title}>300% GDP {t.reward}</Text>

      {rewardClaimed ? (
        <View>
          <Text style={styles.claimedText}>
            {t.You_have_already_claimed_your} 300% GDP {t.reward}.
          </Text>
        </View>
      ) : eligible ? (
        <View>
          <Text style={styles.eligibleText}>
            {t.Congratulations}! {t.You_are_eligible_for_the} 300% GDP {t.reward}!
          </Text>
          ...
          {childrenWithSameGDP.length < 8 && (
            <>
              ...
              <Text style={styles.childrenTitle}>
                {t.qualified} GDP {t.requirements} {t.friend} {t.ecological_relations} (
                {greatGreatGrandchildrenWithSameGDP.length}/16):
              </Text>
        ...
        <View>
          <Text style={styles.ineligibleText}>
            {t.You_are_not_eligible_for_the} 300% GDP {t.reward}
          </Text>

          <Text style={styles.requirementText}>{t.requirements} ({t.either}1 || 2) : </Text>

          <Text style={styles.bulletPoint}>
            • {t.condition} 1: {t.at_least} 6 {t.friend} {t.and} 4 + 8 + 16 {t.friend} {t.ecological_relations} {t.with} {t.equal} GDP
          </Text>

          <Text style={styles.bulletPoint}>
            • {t.condition} 2: {t.at_least} 8 {t.friend} {t.with} {t.equal} GDP
          </Text>
          <Text style={styles.bulletPoint}>
          ...
          {childrenWithSameGDP.length < 8 && (
            <>
              ...
              <Text style={styles.currentStatus}>
                {t.qualified} GDP {t.friend} {t.ecological_relations}:{" "}
                {greatGreatGrandchildrenWithSameGDP.length}
              </Text>
            </>
          )}
        </View>
