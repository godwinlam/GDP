...
interface OneThousandRewardScreenProps {
  user: User;
}

export default function OneThousandRewardScreen({
  user,
}: OneThousandRewardScreenProps) {
  ...
  const [greatGrandchildrenWithSameGDP, setGreatGrandchildrenWithSameGDP] =
    useState<User[]>([]);
  const [
    greatGreatGrandchildrenWithSameGDP,
    setGreatGreatGrandchildrenWithSameGDP,
  ] = useState<User[]>([]);
  const [
    greatGreatGreatGrandchildrenWithSameGDP,
    setGreatGreatGreatGrandchildrenWithSameGDP,
  ] = useState<User[]>([]);
  const [
    greatGreatGreatGreatGrandchildrenWithSameGDP,
    setGreatGreatGreatGreatGrandchildrenWithSameGDP,
  ] = useState<User[]>([]);
  ...
  const checkEligibility = async () => {
    try {
      ...
      if (parentGDPPurchase.rewardClaimed500) {
      ...
      setRewardAmount(parentGDPPrice * 9.0);
      ...
      const matchingGreatGreatGrandchildren: User[] = [];
      const matchingGreatGreatGreatGrandchildren: User[] = [];
      const matchingGreatGreatGreatGreatGrandchildren: User[] = [];
                ...
                if (
                  ...
                  for (const greatGreatGrandchildDoc of greatGreatGrandchildrenSnapshot.docs) {
                    ...
                    if (
                      ...
                      matchingGreatGreatGrandchildren.push({
                        uid: greatGreatGrandchildDoc.id,
                        ...greatGreatGrandchildDoc.data(),
                      } as User);

                      // Get great-great-great-grandchildren for this great-great-grandchild
                      ...
                      for (const greatGreatGreatGrandchildDoc of greatGreatGreatGrandchildrenSnapshot.docs) {
                        ...
                        if (
                          ...
                          matchingGreatGreatGreatGrandchildren.push({
                            uid: greatGreatGreatGrandchildDoc.id,
                            ...greatGreatGreatGrandchildDoc.data(),
                          } as User);

                          // Get great-great-great-great-grandchildren for this great-great-great-grandchild
                          ...
                          for (const greatGreatGreatGreatGrandchildDoc of greatGreatGreatGreatGrandchildrenSnapshot.docs) {
                            ...
                            if (
                              ...
                              matchingGreatGreatGreatGreatGrandchildren.push({
                                uid: greatGreatGreatGreatGrandchildDoc.id,
                                ...greatGreatGreatGreatGrandchildDoc.data(),
                              } as User);
                            }
                          }
                        }
      ...
      setGreatGreatGrandchildrenWithSameGDP(matchingGreatGreatGrandchildren);
      setGreatGreatGreatGrandchildrenWithSameGDP(
      ...
      const condition1 =
        matchingChildren.length >= 16 &&
        matchingGrandchildren.length >= 4 &&
        matchingGreatGrandchildren.length >= 8 &&
        matchingGreatGreatGrandchildren.length >= 16 &&
        matchingGreatGreatGreatGrandchildren.length >= 32 &&
        matchingGreatGreatGreatGreatGrandchildren.length >= 64;
      const condition2 = matchingChildren.length >= 20;
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        await addDoc(transactionRef, {
          ...
          timestamp: new Date(),
          type: "gdp_reward",
          rewardPercentage: 9.0,
          description:
            "1000% GDP Reward for having 16+ children, 4+ grandchildren, 8+ great-grandchildren, 16+ great-great-grandchildren, 32+ great-great-great-grandchildren, and 64+ great-great-great-great-grandchildren with matching GDP",
        });
      });
  ...
  return (
    ...
    >
      <Text style={styles.title}>1000% GDP {t.reward}</Text>

      {rewardClaimed ? (
        <View>
          <Text style={styles.claimedText}>
            {t.You_have_already_claimed_your} 1000% GDP {t.reward}.
          </Text>
        </View>
      ) : eligible ? (
        <View>
          <Text style={styles.eligibleText}>
            {t.Congratulations}! {t.You_are_eligible_for_the} 1000% GDP {t.reward}!
          </Text>
          ...
          {childrenWithSameGDP.length < 20 && (
            <>
              <Text style={styles.childrenTitle}>
                {t.qualified} GDP {t.friend} {t.ecological_relations} ({grandchildrenWithSameGDP.length}/4):
              </Text>
              ...
              <Text style={styles.childrenTitle}>
                {t.qualified} GDP {t.friend} {t.ecological_relations} ({greatGrandchildrenWithSameGDP.length}/8):
              </Text>
              ...
              <Text style={styles.childrenTitle}>
                {t.qualified} GDP {t.friend} {t.ecological_relations} ({greatGreatGrandchildrenWithSameGDP.length}/16):
              </Text>
              {greatGreatGrandchildrenWithSameGDP.map(
                (greatGreatGrandchild) => (
                  <Text key={greatGreatGrandchild.uid} style={styles.childItem}>
                    • {greatGreatGrandchild.username}
                  </Text>
                )
              )}

              <Text style={styles.childrenTitle}>
                {t.qualified} GDP {t.friend} {t.ecological_relations} ({greatGreatGreatGrandchildrenWithSameGDP.length}/32):
              </Text>
              {greatGreatGreatGrandchildrenWithSameGDP.map(
                (greatGreatGreatGrandchild) => (
                  <Text
                    key={greatGreatGreatGrandchild.uid}
                    style={styles.childItem}
                  >
                    • {greatGreatGreatGrandchild.username}
                  </Text>
                )
              )}

              <Text style={styles.childrenTitle}>
                {t.qualified} GDP {t.friend} {t.ecological_relations} ({greatGreatGreatGreatGrandchildrenWithSameGDP.length}/64):
              </Text>
              {greatGreatGreatGreatGrandchildrenWithSameGDP.map(
                (greatGreatGreatGreatGrandchild) => (
                  <Text
                    key={greatGreatGreatGreatGrandchild.uid}
                    style={styles.childItem}
                  >
                    • {greatGreatGreatGreatGrandchild.username}
                  </Text>
                )
              )}
            </>
        ...
        <View>
          <Text style={styles.ineligibleText}>
            {t.You_are_not_eligible_for_the} 1000% GDP {t.reward}
          </Text>
          <Text style={styles.requirementText}>{t.requirements} ({t.either}1 || 2) :</Text>
          <Text style={styles.bulletPoint}>
          • {t.condition} 1: {t.at_least} 16 {t.friend} {t.and} 4 + 8 + 16 + 32 + 64 {t.friend} {t.ecological_relations} {t.with} {t.equal} GDP
          </Text>
          <Text style={styles.bulletPoint}>
          • {t.condition} 2: {t.at_least} 20 {t.friend} {t.with} {t.equal} GDP
          </Text>
          <Text style={styles.bulletPoint}>
          ...
          {childrenWithSameGDP.length < 20 && (
            <>
              ...
              <Text style={styles.currentStatus}>
                {t.qualified} GDP {t.friend} {t.ecological_relations}: {greatGrandchildrenWithSameGDP.length}
              </Text>
              <Text style={styles.currentStatus}>
                {t.qualified} GDP {t.friend} {t.ecological_relations}: {greatGreatGrandchildrenWithSameGDP.length}
              </Text>
              <Text style={styles.currentStatus}>
                {t.qualified} GDP {t.friend} {t.ecological_relations}: {greatGreatGreatGrandchildrenWithSameGDP.length}
              </Text>
              <Text style={styles.currentStatus}>
                {t.qualified} GDP {t.friend} {t.ecological_relations}: {greatGreatGreatGreatGrandchildrenWithSameGDP.length}
              </Text>
            </>
          )}
        </View>
