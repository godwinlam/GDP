...
interface OneFiftyRewardScreenProps {
  user: User;
}

export default function OneFiftyRewardScreen({
  user,
}: OneFiftyRewardScreenProps) {
  ...
  const checkEligibility = async () => {
    try {
      ...
      if (parentGDPPurchase.rewardClaimed150) {
      ...
      setRewardAmount(parentGDPPrice * 0.5);
      ...
      // Check each child's GDP purchase and their children (grandchildren)
      for (const childDoc of childrenSnapshot.docs) {
        ...
        if (
          ...
          // Only check grandchildren if we might need them for eligibility
          if (matchingChildren.length < 4) {
      ...
      const condition1 =
        matchingChildren.length >= 3 && matchingGrandchildren.length >= 4;
      const condition2 = matchingChildren.length >= 4;
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        await addDoc(transactionRef, {
          ...
          description:
            "150% GDP Reward for having 4+ children with matching GDP",
        });
      });
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>150% GDP {t.reward}</Text>

      {rewardClaimed ? (
        <View>
          <Text style={styles.claimedText}>
            {t.You_have_already_claimed_your} 150% GDP {t.reward}.
          </Text>
        </View>
      ) : eligible ? (
        <View>
          <Text style={styles.eligibleText}>
            {t.Congratulations}! {t.You_are_eligible_for_the} 150% GDP {t.reward}!
          </Text>
          ...
          {childrenWithSameGDP.length < 4 && (
            <>
              <Text style={styles.childrenTitle}>
                {t.qualified} GDP {t.requirements} {t.friend} {t.ecological_relations} ({grandchildrenWithSameGDP.length}):
              </Text>
        ...
        <View>
          <Text style={styles.ineligibleText}>
            {t.You_are_not_eligible_for_the} 150% GDP {t.reward}
          </Text>
          <Text style={styles.requirementText}>{t.requirements}({t.either}1 || 2) : </Text>

          <Text style={styles.bulletPoint}>
            • {t.condition} 1: {t.at_least} 3 {t.friend} {t.and} 4 {t.friend} {t.ecological_relations} {t.with} {t.equal} GDP
          </Text>

          <Text style={styles.bulletPoint}>
            • {t.condition} 2: {t.at_least} 4 {t.friend} {t.with} {t.equal} GDP
          </Text>
